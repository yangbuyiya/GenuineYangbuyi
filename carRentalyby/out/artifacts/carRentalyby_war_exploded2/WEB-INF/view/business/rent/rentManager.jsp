        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline" style="padding: 5px">
                <input type="text" name="rentid" autocomplete="off" class="layui-input layui-input-inline"
                       placeholder="请输入出租单号" style="height: 30px;  ">
            </div>
        </div>
        <div class="layui-inline">
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline" style="padding: 5px">
                <input type="text" name="startTime" id="startTime" readonly="readonly"
                       ...
                       placeholder="请输入出租开始时间" style="height: 30px;  ">
            </div>
        </div>
        <div class="layui-inline">
            ...
            <div class="layui-input-inline" style="padding: 5px">
                <input type="text" name="endTime" id="endTime" readonly="readonly"
                       class="layui-input layui-input-inline"
                       placeholder="请输入出租结束时间" style="height: 30px;">
            </div>
        </div>
        <div class="layui-inline">
...
<script type="text/html" id="rentBar">
    {{#  if(d.rentflag == 1){ }}
    <a class="layui-btn layui-btn-green layui-btn-xs   " lay-event="exportRent">导出出租单</a>
    {{#  } else { }}
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "${yby}/rent/loadAllRent?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(rentTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.rentid + '】这个出租单么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${yby}/rent/deleteRent ", {rentid: data.rentid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
                ...
                window.location.href="${yby}/stat/exportRent?rentid="+data.rentid;
            }
        });
